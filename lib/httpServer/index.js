...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        // B部受信時
        this.app.put('/blockchain/blocks/transactions', (req, res) => {
            let requestTransactions = Transactions.fromJson(req.body);

            try {
                node.checkReceivedTransactions(requestTransactions);
                res.status(201).send(requestTransactions);
            } catch (ex) {
                if (ex instanceof TransactionAssertionError) throw new HTTPError(400, ex.message, requestTransactions, ex);
                else throw ex;
            }
            // ステップ2 へ
            node.selectBlockchain();
